...
[Serializable]
public class VRInputAxis
{
    public string ConventionalInputAxisName;
    public TrackedPoseDriver PoseDriver;
}

[AddComponentMenu("VRTeleporter/Teleporter")]
...
public class TeleportVive : MonoBehaviour {
    ...
    public VRInputAxis[] TeleportInputAxes;
    private VRInputAxis activeTeleportAxis;
    ...
    */
    ...
    private VRInputAxis GetActiveTeleportInputAxis(float activeThreshold)
    {
        foreach (var teleportInpuAxis in TeleportInputAxes)
        {
            if (Input.GetAxis(teleportInpuAxis.ConventionalInputAxisName) > activeThreshold)
            {
                return teleportInpuAxis;
            }
        }
        return null;
    }
	...
	void Update ()
    {
        ...
        if(CurrentTeleportState == TeleportState.Teleporting)
        {
            ...
            if(Time.time - TeleportTimeMarker >= TeleportFadeDuration / 2)
            {
                ...
                } else
                {
                    ...
                    Vector3 offset = InputTracking.GetLocalPosition(XRNode.Head);
                    offset.y = 0;
        ...
        {
            Debug.Assert(activeTeleportAxis != null);
            ...
            bool shouldTeleport = (Input.GetAxis(activeTeleportAxis.ConventionalInputAxisName) < 0.95f);
        ...
        else //CurrentTeleportState == TeleportState.None
        {
            ...
            activeTeleportAxis = GetActiveTeleportInputAxis(0.95f);
            if (activeTeleportAxis != null)
            {
                Pointer.transform.parent = activeTeleportAxis.PoseDriver.transform;
